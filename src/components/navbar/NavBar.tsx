...
function NavBar() {
  ...
  const renderIcon = (iconType: string) => {
    switch (iconType) {
      case '/':
        return pathname === '/' ? <HiHome size={30} className="text-white" /> : <HomeIcon size={30} className="text-white" />;
      case '/notifications':
        return pathname === '/notifications' ? <IoMdNotifications size={30} className="text-white" /> : <IoMdNotificationsOutline size={30} className="text-white" />;
      case '/login':
      case '/register':
        return pathname === '/login' || pathname === '/register' ? <BsFillPersonFill size={30} className="text-white" /> : <BsPerson size={30} className="text-white" />;
      default:
        return (
          <>
